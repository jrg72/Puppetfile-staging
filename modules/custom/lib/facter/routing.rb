...
#Internal Gateway
# Expected output: The ip address of the internal net router
Facter.add("network_internal_gw") do
  my_gw = nil
  confine :kernel => :linux
  setcode do
    gw_address = Facter::Util::Resolution.exec('/sbin/ip route get 10.0.0.1/8 | head -1')
    #not all network configurations will have a nexthop. 
    #the ip tool expresses the presence of a nexthop with the word 'via'
    if gw_address.include? ' via '
      my_gw = gw_address.split(/\s+/)[2].match(/^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$/).to_s
